...
public class JobPortalControllerTest {
    ...
    public void createJobTest()
    {
        ...
        Mockito.when(jobPortalServices.createJobImpl(jobDetails)).thenReturn(response);
        Response response1=jobPortalController.createJob(jobDetails);
        assertEquals(200,response1.getStatusCode());

        Mockito.when(jobPortalServices.createJobImpl(jobDetails)).thenThrow(RuntimeException.class);
